...
const BRAMBLE_DEFAULT_PORT = 29910;

// We attempt to use the default port but if it is busy, we offer the user to
// run on a different port. `detect()` Promise resolves to the next free port.
Promise.all([
  choosePort(HOST, DEFAULT_PORT),
  choosePort(HOST, BRAMBLE_DEFAULT_PORT),
]).then(([port, bramblePort]) => {
    if (port == null || bramblePort == null) {
    ...
    if (proxySetting && typeof proxySetting !== 'object') {
      console.log(chalk.red('When specified, "proxy" in package.json must be an object.'));
      return;
    }
